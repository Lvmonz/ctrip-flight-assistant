...
2. 如果脚本返回了 `logged_in` → 告知用户已登录成功，然后进入**流程二**确认搜索参数。
3. 如果脚本返回了二维码图片 (`waiting_scan`) → **立即把二维码图片发给用户**，请他们扫码。
4. **发完二维码后你必须停下来！** 只回复一句"请扫码，扫好后回复我"，然后**什么都不做**，等待用户的下一条消息。
5. **判断用户是否扫码完成**：只有当用户的回复明确表示扫码完成（如"扫好了"、"好了"、"完成"、"ok"、"done"）时，才执行 `node scripts/login.js --check`。如果用户发来的是其他内容（比如重复了之前的需求、问了新问题），**不要当作扫码完成**，应该提醒用户先扫码。
6. `--check` 返回 `logged_in` 后 → 进入**流程二**确认搜索参数（不要直接搜索！）。
