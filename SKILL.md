...
1. 运行 `node scripts/login.js --check` 检查是否已登录
2. 如果已登录 → 告知用户当前登录状态
3. 如果未登录 → 运行 `node scripts/login.js`
   - 脚本会先尝试恢复 Cookie
   - 恢复失败则打开扫码登录页，截图二维码
4. 【极其重要】脚本获取到二维码后会**立刻退出并返回路径**。你必须**立刻用 notify_user 等工具把二维码发给用户**，请他们扫码！
5. 停下来等待用户回复（比如"扫好了"或"完成"）。
6. 收到用户确认后，你必须执行 `node scripts/login.js --check` 检查登录结果，成功后 Cookie 会自动持久化！
...
# 强制扫码登录
node scripts/login.js --qrcode

# 手机号登录
node scripts/login.js --phone 13800138000
```
...
3. 运行搜索脚本:
   ```bash
   node scripts/search-flights.js --from 出发城市 --to 到达城市 --date YYYY-MM-DD
   ```
...
```bash
# 基础搜索
node scripts/search-flights.js --from 杭州 --to 北京 --date 2026-04-05

# 仅直飞
node scripts/search-flights.js --from HGH --to PEK --date 2026-04-05 --direct
...
| 用户反馈二维码过期或失效 | 重新运行 `node scripts/login.js --qrcode` 获取新二维码 |
