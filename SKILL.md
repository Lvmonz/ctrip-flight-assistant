...
1. 从用户消息中提取需要的信息。
2. **【致命约束】** 调用搜索脚本前，必须向用户**确认**以下全部参数（缺少的必须停下来问！）：
   - 👉 **出发城市**
   - 👉 **到达城市**
   - 👉 **出发日期** (YYYY-MM-DD)
   - 👉 **舱位** (经济舱 / 商务舱 / 头等舱)
...
3. 参数齐全后运行搜索:
   ...
   cd /home/node/.openclaw/skills/ctrip-flight && node scripts/search-flights.js \
     ...
     --time "早上|中午|晚上|无所谓" \
     --airport "机场简称(如:首都/大兴/萧山)|无所谓" \
     --largeOnly true \
     --sort price
   ```
   *注意: `--largeOnly true` 只有在用户明确要求大飞机/宽体机时才添加，千万不要加引号！不需要大飞机时直接省略该参数。*

4. **【防幻觉与过滤必读】**:
   - 🚨 **绝不允许“挂羊头卖狗肉”！** 若添加 `--largeOnly true` 后搜索结果 `totalFlights: 0`，你**必须诚实地告诉用户**没有符合条件的航班，并询问是否放宽条件。**绝对禁止**偷偷去掉该参数拿一个“中型机”(如空客321/波音737) 忽悠用户这是“大飞机”！
   - 🚨 **机场名字不要带航站楼！** 过滤机场时只用简称（如“首都”、“浦东”），**千万不要带 T2/T3 等航站楼编号**（除非用户强制要求），否则会导致完全符合机场条件但航站楼不匹配的航班被意外过滤掉（例如搜“首都T3”会把降落在“首都T2”的真实航班过滤成 0 趟）。
